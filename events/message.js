...
module.exports = async (client, message) => {
  ...
  client.logger.log(`message: ${messageText}`);

  if (messageText) {
    slackHook(messageText);
  }
  ...
  if (messageText.indexOf(settings.prefix) === 0) {

    const words = messageText.slice(settings.prefix.length).trim();
    const args = words.split(/\s+/g);
    const command = args.shift().toLowerCase();
    controllers(message, command, args);
  }
